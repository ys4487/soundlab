...
# תדר 852 הרץ עם פער בטא לניקוי ערפל מוחי וחדות (15 הרץ)
freq_left = 852.0
freq_right = 867.0
wave_left = (np.sin(2 * np.pi * freq_left * t) * 0.22).astype(np.float32)
wave_right = (np.sin(2 * np.pi * freq_right * t) * 0.22).astype(np.float32)
...
write('brain_fog_clear.wav', sample_rate, stereo_signal)
print("קובץ ניקוי ערפל מוחי Brain Fog נוצר בהצלחה!")